...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        eSensor = executor.submit(sensor.produce, sensor_reading_bus, 0.01)
        eInterpreter = executor.submit(interpreter.consume_produce, sensor_reading_bus, pos_bus, 0.01)
        eController = executor.submit(controller.consume, pos_bus, 0.01)
